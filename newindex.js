...
function runner(){
    ...
    console.log( colours.fg.white, colours.bg.magenta, `   Blotto Radio Monitor   `, colours.reset, '\n');

    getJSON( sc.url + "/" + sc.path + "?sid=" + sc.sid + '&mode=' + sc.mode + '&pass=' + sc.pass).then( function(resp) {
        ...
        var songtitle = resp.songtitle.trim();

        console.log( colours.fg.yellow, `${songtitle}`, colours.reset, '\n');
    ...
    if( resp.uniquelisteners > 0 ) {
      ...
      for( let listener of result){
        ...
        lookupListener( listener.hostname , function ( err, result ){
            ...
            if( err ){
                console.log( 'lookupListener result ' + err  )
            }
      ...
      var c = resp.listeners == 0 ? colours.fg.red : colours.fg.yellow 

      console.log( ` Connections:`, c, `${resp.uniquelisteners}`, colours.reset );

    }
    
    // show daily total listener count
    todaysListenerCount()
...
function lookupListener( hostname, callback ) {
    ...
    var sql = "SELECT * FROM scBlotto WHERE hostname in(?)";
...
function todaysListenerCount( ){
  var con = mysql.createConnection(db);
  var query = "SELECT count(id) `today` FROM `last24_listeners`";
  let q = con.query( query, (error, result ) =>{
    ...
    var todayCount = result[0].today

    var c = todayCount > 0 ? colours.fg.green : colours.fg.white

    console.log(` Daily Total:`, c, todayCount, colours.reset )
...
function insertMysql( listener ) {
     ...
     var values = [
      ...
      var newSql = "INSERT INTO scBlotto ( hostname, timestamp, connecttime, city, region, country, useragent, referrer, latitude, longitude ) VALUES ( ?,?,?,?,?,?,?,?,?,? ) ON DUPLICATE KEY UPDATE timestamp=?, connecttime=?, useragent=?";
